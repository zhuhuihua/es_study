...
//			System.out.println("sbf.toString() = " + sbf.toString());
	...
	private static void writeHdfs(long c, String file) {
		...
		try {
			...
			for (int i = 0; i < day; i++) {
				...
				String afterNDayStr = DateUtils.getAfterNDayStr(i, fromDay);
				System.out.println(DateUtils.getAfterNDayStr(i, fromDay));
				...
				while (currentCount < c) {
					...
					System.out.println(getData(afterNDayStr));
	...
	private static void writeToHdfs() {
		...
		for (long i = 0; i < count; i += step) {
			...
			if (count <= step) {
				_file = _file + suffix;
				writeHdfs(count, _file);
			} else {
				_file = _file + "_" + i / step + suffix;
				if (i + step >= count) {
					writeHdfs(count - i, _file);
				} else
					writeHdfs(step, _file);
			}
		}
	...
	private static void printHELP() {
		System.out.println(
				"用法说明：[commond] | [options [-c count] [-p path] [-f filename] [-x suffix] [-n singlefilecount]]");
		System.out.println("OPTIONS :");
		System.out.println("-c" + " " + "生成数据的总数量");
		System.out.println("-p" + " " + "默认生成文件的存放路径");
		System.out.println("-f" + " " + "生成文件的文件名,默认为null");
		System.out.println("-x" + " " + "文件后缀，默认为空");
		System.out.println("-n" + " " + "单个文件存储条目数");
		System.out.println("-d" + " " + "日期格式：2014-01-01");
		System.out.println("--help" + " " + "显示帮助说明");
		System.out.println("例子:" + "-c" + " " + 1000 + "：" + "生成1000条数据");
	}
